...
class RMS_norm(nn.Module):
    ...
    def forward(self, x):
        result = F.normalize(x, dim=(1 if self.channel_first else -1)) * self.scale * self.gamma.to(x)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = torch.bmm(q, k.transpose(1, 2)) / (self.dim ** 0.5)
...
class Resample(nn.Module):
    """Resampling layer for upsampling and downsampling"""
...
class Encoder3d(nn.Module):
    ...
    def __init__(
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            # Apply temporal downsampling flags in order
...
class Decoder3d(nn.Module):
    ...
    def __init__(
        ...
        upsamples = []
        for i, (in_dim, out_dim) in enumerate(zip(dims[:-1], dims[1:])):
            # Apply temporal upsampling flags in order
            ...
            upsamples.append(
                Up_ResidualBlock(
                    ...
                    up_flag=i != len(dim_mult) - 1,  # All blocks except the last should upsample
                )
            )
        self.upsamples = nn.Sequential(*upsamples)
...
def count_conv3d(model):
    """Count number of CausalConv3d layers in model"""
    count = 0
...
class WanVAE(nn.Module):
    """
    Wan2.2 VAE with 3D encoder and decoder
    
    Args:
        dim: Base dimension
        dec_dim: Decoder dimension
        z_dim: Latent dimension (16 for Wan2.2)
        dim_mult: Dimension multipliers
        num_res_blocks: Number of residual blocks
        attn_scales: Attention scales
        temperal_downsample: Temporal downsampling flags
    ...
    def forward(self, x, sample_posterior=True):
        ...
        Returns:
            Decoded output
        """
        z = self.encode(x) if not sample_posterior else self.sample(x)
