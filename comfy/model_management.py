...
try:
    if (is_nvidia() or is_amd()) and True:
...
if torch.cuda.is_available() and torch.backends.cudnn.is_available() and True:
...
def unet_dtype(device=None, model_params=0, supported_dtypes=[torch.float16, torch.bfloat16, torch.float32], weight_dtype=None):
    ...
    if memargs.get('fp32_unet', False):
        return torch.float32
    if memargs.get('fp64_unet', False):
        return torch.float64
    if memargs.get('bf16_unet', False):
        return torch.bfloat16
    if memargs.get('fp16_unet', False):
        return torch.float16
    if memargs.get('fp8_e4m3fn_unet', False):
        return torch.float8_e4m3fn
    if memargs.get('fp8_e5m2_unet', False):
        return torch.float8_e5m2
    if memargs.get('fp8_e8m0fnu_unet', False):
...
def text_encoder_dtype(device=None):
    if memargs.get("fp8_e4m3fn_text_enc",False):
        return torch.float8_e4m3fn
    elif memargs.get("fp8_e5m2_text_enc",False):
        return torch.float8_e5m2
    elif memargs.get("fp16_text_enc",False):
        return torch.float16
    elif memargs.get("bf16_text_enc",False):
        return torch.bfloat16
    elif memargs.get("fp32_text_enc",False):
        return torch.float32
...
def vae_dtype(device=None, allowed_dtypes=[]):
    if memargs.get("fp16_vae", False):
        return torch.float16
    elif memargs.get("bf16_vae", False):
        return torch.bfloat16
    elif memargs.get("fp32_vae", False):
        return torch.float32
