...
conditioning_set_values = lambda *x, **y: print("fixme")
...
def default_should_register(hook: Hook, model: ModelPatcher, model_options: dict, target_dict: dict[str], registered):
...
class Hook:
    ...
    def should_register(self, model: ModelPatcher, model_options: dict, target_dict: dict[str], registered):
        return self.custom_should_register(self, model, model_options, target_dict, registered)

    def add_hook_patches(self, model: ModelPatcher, model_options: dict, target_dict: dict[str], registered):
...
def create_transformer_options_from_hooks(model: ModelPatcher, hooks,  transformer_options: dict[str]=None):
...
def _combine_hooks_from_values(c_dict: dict[str, None], values: dict[str, None], cache: dict[tuple[None, None], None]):
    hooks_key = 'hooks'
...
def conditioning_set_values_with_hooks(conditioning, values={}, append_hooks=True,
                                       cache: dict[tuple[None, None], None]=None):
    c = []
    if cache is None:
        cache = {}
...
def set_hooks_for_conditioning(cond, hooks, append_hooks=True, cache: dict[tuple[None, None], None]=None):
    if hooks is None:
        return cond
...
def set_conds_props(conds: list, strength: float, set_cond_area: str,
                   mask: torch.Tensor=None, hooks=None, timesteps_range: tuple[float,float]=None, append_hooks=True):
    final_conds = []
    cache = {}
    for c in conds:
...
def set_default_conds_and_combine(conds: list, new_conds: list,
                                   hooks=None, timesteps_range: tuple[float,float]=None, append_hooks=True):
    combined_conds = []
    cache = {}
    for c, new_c in zip(conds, new_conds):
        # first, apply lora_hook to new conditioning, if provided
